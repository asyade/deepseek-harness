...
| `@deepseek-ai/dsh-tool-lsp-diagnostics` | `lsp-diagnostics` | `ctx.tools`, `ctx.lspDiagnostics`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | - |
...
拉取工作区或单个文件的当前诊断快照。返回有上限的诊断（默认仅错误，5 个文件×每文件 20 条）。写入后调用可检查编译错误，无需重新发现问题。
