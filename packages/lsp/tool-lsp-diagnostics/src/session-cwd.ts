/**
 * Derive the workspace root an `lsp-diagnostics` call resolves against from the calling
 * agent's session. A missing cwd fails as `LSP_WORKSPACE_REQUIRED` because the
 * diagnostics provider must canonicalize a real workspace before querying.
 * @module @deepseek-ai/dsh-tool-lsp-diagnostics/session-cwd
 */
