...
| `@deepseek-ai/dsh-tool-lsp-diagnostics` | `lsp-diagnostics` | `ctx.tools`、`ctx.lspDiagnostics`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | lsp-diagnostics 工具将诊断选择与快照生命周期置于 ctx.lspDiagnostics 之后，因此其模型可见 schema 在更换提供方时保持稳定。运行时要求已注册提供方，例如 `@deepseek-ai/dsh-lsp-stdio-diagnostics`；如果没有提供方，拉取会返回结构化 `LSP_UNAVAILABLE` 错误，而不会改变 schema。 |
...
拉取工作区或单个文件的当前诊断快照。返回截断后的诊断（默认仅错误，5 文件 × 每文件 20 条）。写入后用于检查编译错误，无需重新发现它们。
...
来源：[`packages/lsp/tool-lsp-diagnostics/src/index.ts`](../packages/lsp/tool-lsp-diagnostics/src/index.ts)
